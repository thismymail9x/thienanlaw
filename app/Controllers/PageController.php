...
class PageController extends BaseController
{
    public function index()
    {
        ...
        if ($this->request->getVar('for_search') != false) {
            ...
            if ($this->request->getVar('for_menu') != false) {
                if (isset($_SESSION['page.page']) == true) {
                    session()->remove('page.page');
                }
        ...
        $postsModel->search($allParams,false,true);

        $totals = $postsModel->countAllResults(false);
        $countPages = $this->utils->getCountPages($totals);
        //get current page number

        if ($this->request->getVar('page_page') != false) {
            //changed page => set session
            $currentPageNumber = $this->request->getVar('page_page');
            $_SESSION['page.page'] = $currentPageNumber;
        } else {
            if (session()->get('page.page') == true
                && $this->request->getVar('for_menu') == false) {
                $sessionPage = session()->get('page.page');
    ...
    public function register()
    {
        $data = [];
        $postsModel = new PostsModel();
        $data['title_page'] = 'Thêm mới trang tĩnh';
        $languagesCodeModel = new LanguagesCodeModel();
        $seosModel = new SeosModel();
        $data['langCode'] = $languagesCodeModel->getByConditions(['deleted_at'=>null]);
        // id cua danh mục page = 22011302041255 là 1 số cố định
        $postCategory = $postsModel->getById(PAGE_ID);
        if ($this->request->getMethod() == 'post') {
            ...
            if ($is_error_validate == true) {
                ...
                return view('Admin/Page/register', $data);
            }
            ...
            return redirect()->to(base_url('page-admin?for_menu=1'));
        }
        return view('Admin/Page/register', $data);
    }
    ...
    public function register_home()
    {
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            if ($is_error_validate == true) {
                $data["post"] = $dataForms;
                session()->setFlashdata('msg_error_register_post', 'Lỗi');
                return view('Admin/Page/register_home', $data);
            }
